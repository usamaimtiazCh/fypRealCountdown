...
import { funcuserState } from "../../Redux/loginslice";
export const Header = () => {
  const navigate = useNavigate();
  const [activestate, setactivestate] = React.useState("Home");
  const menuItem = ["Home", "Properties", "Contact-Us"];
                  ...
                  <ul className="space-y-6 tracking-wide font-medium text-base lg:text-sm lg:flex lg:space-y-0">
                    {menuItem?.map((data) => (
                      <li key={data}>
                        <p
                          onClick={() => {
                            navigate(`/${data === "Home" ? "" : data}`);
                            setactivestate(data);
                          }}
                          className={`${
                            activestate === data ? "text-blue-950" : ""
                          } cursor-pointer block md:px-4 transition hover:text-blue-950 dark:hover:text-primaryLight focus:text-blue-950`}
                        >
                          <span>{data}</span>
                        </p>
                      </li>
                    ))}
                    {ReduxData?.loginslice?.islogedin ? (
                      ReduxData?.loginslice?.userState === "seller" ||
                      ReduxData?.loginslice?.userState === "buyer" ? (
                        <li>
                          <p
                            onClick={() => {
                              dispatch(
                            ...
                            className={`${
                              activestate === "user" ? "text-blue-950" : ""
                            } cursor-pointer block md:px-4 transition hover:text-blue-950 dark:hover:text-primaryLight focus:text-blue-950`}
                          >
                            <span>
                              {ReduxData?.loginslice?.userState === "seller"
                  ...
                  {ReduxData?.loginslice?.islogedin ? (
                    ...
                    <>
                      <p
                        ...
                        className="cursor-pointer relative flex h-9 ml-auto items-center justify-center sm:px-6 before:absolute before:inset-0 before:rounded-full focus:before:bg-sky-600/10 dark:focus:before:bg-sky-400/10 before:transition before:duration-300 hover:before:scale-105 active:duration-75 active:before:scale-95"
                      >
                        <span className="relative text-sm font-semibold text-blue-950 dark:text-primaryLight">
                          Sign Up
                        </span>
                      </p>
                      <p
                        ...
                        className="cursor-pointer relative flex h-9 ml-auto items-center justify-center sm:px-6 before:absolute before:inset-0 before:rounded-full before:bg-sky-600 dark:before:bg-sky-400 before:transition before:duration-300 hover:before:scale-105 active:duration-75 active:before:scale-95"
                      >
